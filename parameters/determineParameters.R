#Simulates data for 1000 SNPs with different values for p1
#!!!!!!!!!!!!!!!!!!!!!
library(updog)
library(dplyr)
library(seqinr)
...
for(p1 in seq_along(changeIndvProbBinomVec)){
  changeIndvProbBinom <- changeIndvProbBinomVec[p1]
  VCFnam <- paste0(VCFnam0, "_", changeIndvProbBinom)
...
#SAVE PARAMETERS
  ...
  rm(list = ls()[!ls() %in% c("rGenome0", "parameters", names(parameters), "changeIndvProbBinomVec")])
